...
use App\Modules\Warehouse\Models\ChemicalStockOpnameModel;
use CodeIgniter\HTTP\RedirectResponse;

class ChemicalStockOpnameController extends BaseController
{
    protected ChemicalStockOpnameModel $model;

    public function __construct()
    {
        $this->model = new ChemicalStockOpnameModel();
    }

    public function index(): string|RedirectResponse
    {
        if (!canDo('warehouse.stock_opname.view')) return $this->forbidden();

        return view('App\Modules\Warehouse\Views\stock_opname\index', [
            'title'            => 'Stock Opname Kimia',
            'page_title'       => 'Stock Opname Bahan Kimia',
            'page_description' => 'Catat hasil hitung fisik stok kimia murni (Available) di akhir periode, untuk dibandingkan dengan catatan sistem',
    ...
    public function grid()
    {
        ...
        if (!canDo('warehouse.stock_opname.view')) return $this->jsonError('Akses ditolak', 403);
    ...
    {
        if (!$this->request->isAJAX()) return $this->jsonError('Method not allowed', 405);
        if (!canDo('warehouse.stock_opname.create')) return $this->jsonError('Akses ditolak', 403);
    ...
    private function breadcrumbs(): array
    {
        return [
            ...
            ['name' => 'Stock Opname Kimia', 'active' => true],
        ];
    }
