...
class ChemicalModel extends Model
{
    ...
    public function forceDeleteData(int $id): array
    {
        ...
        $used = $this->db->table('chemical_stock_openings')->where('chemical_id', $id)->countAllResults();
        if ($used > 0) {
            return ['status' => 'error', 'message' => "Bahan kimia tidak dapat dihapus permanen karena sudah memiliki {$used} data stok awal"];
        }
