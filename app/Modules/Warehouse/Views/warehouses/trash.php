...
<script>
    const WarehouseTrash = {
        ...
        async restore(id, name) {
            ...
            try {
                const res = await this.post(this.BASE + `warehouse/master/warehouses/restore/${id}`, new FormData());
        ...
        async forceDelete(id, name) {
            ...
            if (!r.isConfirmed) return;
            try {
                const res = await this.post(this.BASE + `warehouse/master/warehouses/force-delete/${id}`, new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.toast('success', res.message);
                } else this.toast('error', res.message);
