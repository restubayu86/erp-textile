...
<style>
    ...
    #category-table_wrapper .top {
        display: flex;
        justify-content: center;
        margin-bottom: 1rem;
    }

    #category-table_wrapper .top input {
        width: 300px;
        border-radius: 20px;
    ...
    #category-table_wrapper .bottom .dataTables_length {
        flex: 1;
        text-align: left;
        order: 1;
    }

    #category-table_wrapper .bottom .dataTables_paginate {
        flex: 1;
        text-align: center;
        order: 2;
    }

    #category-table_wrapper .bottom .dataTables_info {
        flex: 1;
        text-align: right;
        order: 3;
    }
    ...
    #category-table_wrapper .dataTables_paginate .paginate_button {
        padding: 0.375rem 0.75rem;
        margin: 0 0.25rem;
        border-radius: 0.375rem;
    }

    #category-table_wrapper .dataTables_paginate .paginate_button.current {
...
<div class="w-100">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 gap-2 flex-wrap no-print">
        <div class="d-flex gap-2">
            <?php if (canDo('warehouse.chemical_categories.delete')): ?>
                <a href="<?= site_url('warehouse/master/chemical-categories/trash') ?>" class="btn btn-subtle-danger btn-sm">
        ...
        <div class="d-flex gap-2">
            ...
            <?php if (canDo('warehouse.chemical_categories.create')): ?>
                <button class="btn btn-primary btn-sm" id="btn-create" type="button">
                    <span class="fas fa-plus me-1"></span>Tambah Kategori
                </button>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="print-header mb-3">
        <h5 class="fw-bold mb-1">Daftar Kategori Bahan Kimia</h5>
    ...
    <div class="mx-n4 px-4 mx-lg-n6 px-lg-6 bg-body-emphasis py-5 border-y">
        <table class="table table-hover fs-9 nowrap align-middle" id="category-table">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Kategori</th>
        ...
        <div class="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
            <div class="modal-header border-bottom py-3 px-4" id="modal-header">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="modal-title fw-bold mb-0" id="modal-title">Tambah Kategori</h5>
                        <p class="text-muted fs-10 mb-0" id="modal-subtitle">Buat data kategori baru</p>
                    </div>
                </div>
                ...
                <div class="card border mb-0" style="border-radius:.75rem">
                    <div class="card-body p-3">
                        <p class="fs-10 fw-bold text-uppercase text-primary mb-3" style="letter-spacing:.08em">
                            <span class="fas fa-clipboard-list me-1"></span>Informasi Kategori
                        </p>
                        <div class="row g-3">
                            <div class="col-md-7">
                                ...
                                <input type="text" class="form-control form-control-sm" id="f-name"
                                    placeholder="Nama kategori" maxlength="100" autocomplete="off">
                                <div class="invalid-feedback" id="err-category_name"></div>
                            </div>
                            <div class="col-md-5">
                                ...
                                <input type="text" class="form-control form-control-sm font-monospace fw-bold"
                                    id="f-code" placeholder="Kode kategori" maxlength="30" autocomplete="off"
                                    style="text-transform:uppercase;letter-spacing:.06em">
                                <div class="form-text fs-10">Maks. 30 karakter</div>
                                <div class="invalid-feedback" id="err-category_code"></div>
                            </div>
                            <div class="col-md-5">
...
<script>
    const CAN_EDIT_CATEGORY = <?= json_encode(canDo('warehouse.chemical_categories.edit')) ?>;
    ...
    const Category = {
        ...
        async loadStats() {
            try {
                const d = await this.get(this.BASE + 'warehouse/master/chemical-categories/stats');
        ...
        initDatatable() {
            const self = this;
            this.dt = $('#category-table').DataTable({
                responsive: true,
                scrollX: true,
                processing: true,
                serverSide: true,
                ...
                ajax: {
                    url: this.BASE + 'warehouse/master/chemical-categories/datatables',
                    type: 'GET',
                    data: d => {
                ...
                columns: [{
                    ...
                    },
                    {
                        data: 'chemical_count',
                        render: d => Number(d) > 0 ?
                            `<span class="badge badge-phoenix badge-phoenix-info p-2 fs-10">${self.e(d)} bahan</span>` : '<span class="text-muted fst-italic">—</span>'
                    },
                    {
                        data: 'status',
                    ...
                    },
                    {
                        ...
                        render: (d, t, r) => {
                            const edit = CAN_EDIT_CATEGORY ?
                                `<button class="btn btn-subtle-primary btn-sm btn-edit" data-id="${r.id}"><span class="fas fa-pencil-alt"></span></button>` :
                                '';
                            const del = CAN_DELETE_CATEGORY ?
                                `<button class="btn btn-subtle-danger btn-sm btn-delete" data-id="${r.id}" data-name="${self.e(r.category_name)}"><span class="fas fa-trash"></span></button>` :
                                '';
        ...
        openCreate() {
            ...
            document.getElementById('modal-subtitle').textContent = 'Buat data kategori baru';
        ...
        async openEdit(id) {
            ...
            document.getElementById('modal-subtitle').textContent = 'Perbarui data kategori';
            document.getElementById('save-text').textContent = 'Update';
            this.setLoading(true);
            new bootstrap.Modal(document.getElementById('categoryModal')).show();
            try {
                const d = await this.get(this.BASE + `warehouse/master/chemical-categories/${id}`);
                if (d.status === 'success' && d.data) {
                    document.getElementById('f-name').value = d.data.category_name ?? '';
                    document.getElementById('f-code').value = d.data.category_code ?? '';
                    ...
                    if (d.data.category_name) this.markValid('f-name');
                    if (d.data.category_code) this.markValid('f-code');
        ...
        async save() {
            ...
            fd.set('category_name', document.getElementById('f-name').value.trim());
            fd.set('category_code', document.getElementById('f-code').value.trim().toUpperCase());
            ...
            this.setLoading(true);
            try {
                const res = await this.post(this.BASE + 'warehouse/master/chemical-categories/store', fd);
        ...
        clearErrors() {
            document.querySelectorAll('#categoryModal .is-invalid, #categoryModal .is-valid').forEach(el => {
                el.classList.remove('is-invalid', 'is-valid');
            });
            document.querySelectorAll('#categoryModal .invalid-feedback').forEach(el => {
                el.textContent = '';
                el.style.visibility = '';
            });
            document.getElementById('modal-alert').classList.add('d-none');
        ...
        showErrors(errors) {
            const map = {
                category_name: ['f-name', 'err-category_name'],
                category_code: ['f-code', 'err-category_code'],
        ...
        async deleteItem(id, name) {
            const result = await Swal.fire({
                ...
                html: `<strong>${name}</strong> akan dipindahkan ke sampah.<br><small class="text-muted">Dapat dipulihkan dari menu Sampah. Kategori yang masih dipakai bahan kimia tidak bisa dihapus.</small>`,
                icon: 'warning',
            ...
            try {
                const res = await this.post(this.BASE + `warehouse/master/chemical-categories/${id}/delete`, new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.loadStats();
                    this.toast('success', res.message);
        ...
        fmtUser(name, employeeName = null) {
            ...
            // Jika employeeName null atau undefined, tampilkan username
            if (!employeeName) {
