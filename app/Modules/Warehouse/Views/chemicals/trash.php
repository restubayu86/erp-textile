...
<div class="w-100">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 gap-2 flex-wrap">
        <a href="<?= site_url('warehouse/master/chemicals') ?>" class="btn btn-subtle-secondary btn-sm">
    ...
    <div class="alert alert-subtle-warning py-2 px-3 fs-9 mb-3">
        ...
        Data di sini akan tetap tersimpan sampai dipulihkan atau dihapus permanen. Menghapus permanen bahan kimia juga akan menghapus seluruh variannya.
    </div>
...
<script>
    ...
    const ChemicalTrash = {
        ...
        initDatatable() {
            ...
            this.dt = $('#trash-table').DataTable({
                ...
                ajax: {
                    url: this.BASE + 'warehouse/master/chemicals/trash-datatables',
                    type: 'GET',
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        className: 'text-end',
                        render: (d, t, r) => {
                            const restore = CAN_DELETE_CHEMICAL ?
                                `<button class="btn btn-subtle-success btn-sm btn-restore" data-id="${r.id}" data-name="${self.e(r.chemical_name)}" title="Pulihkan">
                                    <span class="fas fa-trash-restore"></span>
                                </button>` : '';
                            const forceDel = CAN_DELETE_CHEMICAL ?
                                `<button class="btn btn-subtle-danger btn-sm btn-force-delete" data-id="${r.id}" data-name="${self.e(r.chemical_name)}" title="Hapus Permanen">
        ...
        async restoreItem(id, name) {
            ...
            try {
                const res = await this.post(this.BASE + `warehouse/master/chemicals/${id}/restore`, new FormData());
        ...
        async forceDeleteItem(id, name) {
            const result = await Swal.fire({
                ...
                html: `<strong>${name}</strong> beserta seluruh variannya akan dihapus permanen dan <u>tidak bisa dipulihkan lagi</u>.`,
                icon: 'error',
                showCancelButton: true,
                reverseButtons: true,
            ...
            if (!result.isConfirmed) return;
            try {
                const res = await this.post(this.BASE + `warehouse/master/chemicals/${id}/force-delete`, new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.toast('success', res.message);
                } else {
        ...
        async emptyTrash() {
            const result = await Swal.fire({
                ...
                html: `Semua bahan kimia di sampah beserta variannya akan dihapus permanen dan <u>tidak bisa dipulihkan lagi</u>.`,
                icon: 'error',
                showCancelButton: true,
                reverseButtons: true,
                confirmButtonColor: '#e63757',
            ...
            if (!result.isConfirmed) return;
            try {
                const res = await this.post(this.BASE + 'warehouse/master/chemicals/empty-trash', new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.toast('success', res.message);
                } else {
    ...
    $(document).ready(() => ChemicalTrash.init());
</script>
<?= $this->endSection() ?>