...
<div class="w-100">
    ...
    <div class="alert alert-subtle-warning d-flex align-items-start gap-2 py-2 mb-3">
        ...
        <span class="small">
            Departemen di sampah <strong>tidak aktif</strong>.
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 gap-2 flex-wrap">
        <a href="<?= site_url('hrm/departments') ?>" class="btn btn-subtle-secondary btn-sm">
    ...
    const DeptTrash = {
        ...
        initDatatable() {
            ...
            this.dt = $('#trash-table').DataTable({
                ...
                ajax: {
                    url: this.BASE + 'hrm/departments/trash-datatables',
                    type: 'GET',
                    ...
                    },
                    {
                        ...
                        render: (d, t, r) => `
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-subtle-success btn-restore" data-id="${r.id}" data-name="${self.e(r.department)}" title="Pulihkan">
                                    <span class="fas fa-rotate-left"></span>
                                </button>
                                <button class="btn btn-subtle-danger btn-force-delete" data-id="${r.id}" data-name="${self.e(r.department)}" title="Hapus Permanen">
        ...
        async restore(id, name) {
            ...
            try {
                const res = await this.post(this.BASE + `hrm/departments/restore/${id}`, new FormData());
        ...
        async forceDelete(id, name) {
            ...
            if (!r.isConfirmed) return;
            try {
                const res = await this.post(this.BASE + `hrm/departments/force-delete/${id}`, new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.toast('success', res.message);
                } else this.toast('error', res.message);
        ...
        async emptyTrash() {
            ...
            const r = await Swal.fire({
                ...
                html: `${count} departemen akan dihapus <strong>permanen</strong>.<br><span class="text-danger small">Tidak bisa dibatalkan.</span>`,
                icon: 'error',
                showCancelButton: true,
                reverseButtons: true,
                confirmButtonColor: '#e63757',
            ...
            if (!r.isConfirmed) return;
            try {
                const res = await this.post(this.BASE + 'hrm/departments/empty-trash', new FormData());
                if (res.status === 'success') {
                    this.dt.ajax.reload(null, false);
                    this.toast('success', res.message);
                } else this.toast('error', res.message);
